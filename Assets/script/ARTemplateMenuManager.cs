...
public class ARTemplateMenuManager : MonoBehaviour
{
    ...
    [SerializeField]
    GameObject m_attached_object;

    [SerializeField]
    TouchMangerScript m_touch_manger;
    ...
    {
        get => m_attached_object;
        set => m_attached_object = value;
    }
    ...
    void DeleteFocusedObject()
    {
        ...
        {
            if (currentFocusedObject.transform.gameObject.tag == "rocket")
            {
                Vector3 pos = currentFocusedObject.transform.gameObject.transform.position;
